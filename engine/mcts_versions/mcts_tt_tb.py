...
class MCTS:
    ...
    def _evaluate_leaf(self, board: chess.Board) -> float:
        if self.tb is not None and len(board.piece_map()) <= 5:
            try:
                ...
                return float(np.clip(wdl / 2.0, -1.0, 1.0))
    ...
    def _expand(self, node: Node):
        ...
        if key in self.cache and "policy" in self.cache[key]:
            probs = self.cache[key]["policy"]
        else:
        ...
        for _ in range(num_simulations):
            ...
            if node.board.is_game_over():
                ...
                if entry and "value" in entry:
                    ...
                    self.cache.setdefault(key, {})["value"] = float(value)
