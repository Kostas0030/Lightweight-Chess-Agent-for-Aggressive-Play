...
class MCTSBatch:
    ...
    # --------------------------------------------------------
    def _expand_node(self, node: Node, probs: np.ndarray, aggression_bonus=1.3):
        total_prob = 0.0
        ...
        for move in board_ref.legal_moves:
            ...
            board_ref.push(move)
            if board_ref.is_check() or board_ref.is_capture(move):
                prob *= aggression_bonus
    ...
    # Backpropagation
    # --------------------------------------------------------
    def _backpropagate(self, path, value, gamma=0.97):
        leaf_depth = len(path) - 1
        discount = gamma ** leaf_depth  # fixed for the entire path
        value *= discount
